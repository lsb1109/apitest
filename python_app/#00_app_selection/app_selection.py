...
class app_selection(QDialog):
    ...
    def initUI(self):
        ...
        for i in type_icon_img:
            ...
            self.tab_app_list.currentItemChanged.connect(self.selected_app)
        ...
        self.update_btn = self.btn_create(update_icon, 28, self.app_list_build)

        self.sub_layout1.addStretch(1)
        self.sub_layout1.addWidget(self.update_btn)

        # independent layout
        run_icon = self.icon_create(run_img)
        self.run_btn = self.btn_create(run_icon, 24, self.ok_fun)
        self.run_btn.setStyleSheet(custom_style)
        all_items_layout_list[tab_num][app_num].addWidget(self.run_btn)

        download_icon = self.icon_create(download_img)
        self.download_btn = self.btn_create(download_icon, 24, self.download_fun)
    ...
    def selected_app(self):
        ...
        try:
            try:
                all_items_layout_list[tab_num][app_num].removeWidget(
                    self.run_btn)
            except:
                pass
            app_num = tab_listwidget_list[tab_num].currentRow()
            if tab_num == 1:
                all_items_layout_list[tab_num][app_num].addWidget(
                    self.download_btn)
            all_items_layout_list[tab_num][app_num].addWidget(self.run_btn)
    ...
    def app_list_build(self):
        ...
        for i in range(len(type_icon_img)):
            ...
            for j in temp_contents_list:
                item = QListWidgetItem(tab_listwidget_list[i])
                temp_item_widget = QWidget()
                temp_item_widget.setStyleSheet("background: transparent;")
                temp_item_layout = QHBoxLayout(temp_item_widget)
                temp_item_layout.setContentsMargins(6, 0, 4, 0)
                temp_item_label = QLabel()
                temp_item_label.setText(j)
                temp_item_layout.addWidget(temp_item_label)
                temp_item_layout.setStretchFactor(temp_item_label, 1)
                tab_listwidget_list[i].setItemWidget(item, temp_item_widget)
                temp_item_layout_list.append(temp_item_layout)
            all_contents_list.append(temp_contents_list)
            all_items_layout_list.append(temp_item_layout_list)
    ...
    def btn_create(self, icon, size, funtion):
