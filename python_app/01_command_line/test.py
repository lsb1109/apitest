...
py_drive_url = "https://github.com/kh1012/python-examples"
...
class app_selection(QDialog):
    ...
    def initUI(self):
        ...
        for i in tab_list:
            ...
            temp_pixmap = QPixmap()
            temp_img_path = img_drive_url + type_icon_img[tab_list.index(i)]
            temp_img = urllib.request.urlopen(temp_img_path).read()
        ...
        update_pixmap = QPixmap()
        temp_img_path = img_drive_url + update_img
        temp_img = urllib.request.urlopen(temp_img_path).read()
        update_pixmap.loadFromData(temp_img)
        update_icon = QIcon()
        update_icon.addPixmap(update_pixmap)
...
class manual_dialog(QDialog):
    ...
    def initUI(self):
        ...
        self.web_widget.setUrl(
            QUrl("http://127.0.0.1:5500/html_temp_" + str(tab_num) + "/test_html" + str(app_num) + "/index.html"))
        self.web_widget.setHidden(True)

        # 2 layout (UI default)
        href_pixmap = QPixmap()
        temp_img_path = img_drive_url + href_img
        temp_img = urllib.request.urlopen(temp_img_path).read()
        href_pixmap.loadFromData(temp_img)
        href_icon = QIcon()
        href_icon.addPixmap(href_pixmap)
        ...
        self.manual_link.setText(" Open With Browser")
    ...
    def selection_changed(self, msg):
        if tab_num == 2:
